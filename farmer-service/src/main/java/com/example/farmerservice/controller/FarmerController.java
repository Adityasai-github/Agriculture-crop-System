...
public class FarmerController {
	...
	GetAllFarmers getAllFarmers;
	  ...
	  @PostMapping("/auth")
	  private ResponseEntity<String> authfarmer(@RequestBody Farmer farmer) {
	  return farmerService.authFarmer(farmer);
	  }
	...
	public void giveRating(@RequestBody Farmer farmer, @PathVariable String ID) {
		farmer.setID(ID);
		Integer rating = farmer.getAllRatings().stream().mapToInt(Integer::intValue).sum()
				/ farmer.getAllRatings().size();
	 ...
	 * boolean isFarmerExist = repo.existsById(ID); if (isFarmerExist) {
	 * farmerService.deleteById(ID);
	 * 
	 * return new ResponseEntity<Object>("farmer deleted with id" + ID,
	 * HttpStatus.OK); } else { throw new
	 * ApiException("CAN NOT DELETE FARMER ,AS FARMER NOT FOUND WITH THIS ID ::"); }
	 * }
	 */
